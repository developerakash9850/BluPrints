...
public class AEMScrybeDevice {
	...
	public boolean connectToPrinter(String printerName) throws IOException {
		...
		//BluetoothDevice device = getPrinterByAddress(printerName);
	...
	private BluetoothDevice getPrinterByAddress(String printerName) {
		Set<BluetoothDevice> pairedDevices = BluetoothAdapter.getDefaultAdapter().getBondedDevices();
		for (BluetoothDevice device : pairedDevices) {
			if (device.getAddress() == null)
				continue;
			if (device.getAddress().contains(printerName)) {
				remoteDevice = device;
				return device;
			}
			else {
	...
	public ArrayList<String> getPairedPrinterByName() {
	...
	public ArrayList<String> getPairedPrinterByAddress() {
		if (!BluetoothAdapter.getDefaultAdapter().isEnabled()) {
			BluetoothAdapter.getDefaultAdapter().enable();
			try {
				Thread.sleep(3000);
		...
		for (BluetoothDevice device : pairedDevices) {
			...
			/*if ((device.getAddress().contains("rinter")))
				 pairedPrinters.add(device.getAddress());
